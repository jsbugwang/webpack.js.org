...
以下部分包含编辑(edit)和格式化(format)本网站内容的所有必需知识。请确保在开始编辑或添加之前，你已经进行过一些研究。有时候最困难的地方在于，确定内容应该在哪和确定它是否存在。
...
## YAML 文件顶部信息(frontmatter) $#yaml-frontmatter$
...
- Webpack 作为句子的第一个单词时，W 可以大写。([source](https://github.com/webpack/media#name))
...
### 配置(configuration)属性 $#configuration-properties$

[配置(configuration)](/configuration) 属性，应该按字母顺序排序：
...
### 假设以及简单(assumptions and simplicity) $#assumptions-and-simplicity$
...
请不要假设事情简单。避免使用词汇 '只是'('just')，'简单地'('simply')。
