...
## Compiler 和 Compilation $#Compiler和Compilation$
...
class FileListPlugin {
  ...
  // 需要传入自定义插件构造函数的任意选项（这是自定义插件的公开API）
  constructor(options = {}) {
    // 在应用默认选项前，先应用用户指定选项
    // 合并后的选项暴露给插件方法
    // 记得在这里校验所有选项
...
每一个事件钩子都预先定义为同步、异步、瀑布、并行钩子，钩子在内部用 call/callAsync 方法调用。支持的钩子清单或可被绑定的钩子清单，通常在 `this.hooks` 属性指定。
...
- **Async Series(异步串联) Hook**
  ...
  插件处理函数(handler functions)的参数为所有参数，以及一个签名为 `(err?: Error) -> void` 的 callback 函数，callback 函数的 。处理函数按注册顺序执行。`callback` 在所有处理函数执行完后调用。这是 `emit` 和 `run` 事件的常见使用模式。
