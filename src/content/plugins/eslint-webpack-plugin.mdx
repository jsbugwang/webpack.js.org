...
你可以传入 [eslint options](https://eslint.org/docs/developer-guide/nodejs-api#%e2%97%86-new-eslint-options) 参数。

注意你提供的配置选项会传给 `ESLint` 类。
这是一组和你在 `package.json` 与 `.eslintrc` 所指定选项不同的选项。
查阅 [eslint docs](https://eslint.org/docs/developer-guide/nodejs-api#%e2%97%86-new-eslint-options) 获取更多详情。

**警告**: 在 eslint-webpack-plugin 版本 1 ，需要把选项传给已废弃的 [CLIEngine](https://eslint.org/docs/developer-guide/nodejs-api#cliengine)。
...
用于 linting 的 `eslint` 实例的路径。如果 `eslintPath` 是类似官方 eslint 的目录（If the `eslintPath` is a folder like a official eslint），或者指定了 `formatter` 选项，那么就不需要安装 `eslint` 了。
...
启用 [ESLint autofix feature](https://eslint.org/docs/developer-guide/nodejs-api#%e2%97%86-eslint-outputfixes-results)。
...
任何警告都会导致模块构建（module build）失败，设置为 `false` 禁用。
